...
/**
 * Thread-safe session service - vervangt de Sessie singleton. Gebruik dit als equivalent van
 * useSession() in Next.js. Injecteer via @Autowired of constructor injection in elke
 * controller/service.
 */
@Service
...
public class SessieService {
    ...
    /**
     * Haalt de ingelogde werknemer op uit de SecurityContext. Elke request heeft zijn eigen
     * SecurityContext (thread-safe).
     */
    ...
    public boolean isInZelfdeTeam(Integer werknemerId) {
        ...
        Set<Integer> mijnTeamIds =
                teamwerknemerRepository.findByWerknemerId(ingelogdId).stream()
    ...
    public boolean isLidVanTeam(Integer teamId) {
        return teamwerknemerRepository.existsByTeamIdAndWerknemerId(
                teamId, getIngelogdeWerknemerId());
    }
     ...
     * Admin: altijd toegang Manager: toegang behalve tot andere managers (tenzij admin) Supervisor:
     * alleen als ze in hetzelfde team zitten Werknemer: alleen eigen data
     */
    public void assertToegangTotWerknemer(Integer werknemerId) {
        ...
        if ("Manager".equals(ingelogd.getRol())) {
            Werknemer doelWerknemer =
                    werknemerRepository
                            .findById(werknemerId)
                            .orElseThrow(
                                    () -> new AccessDeniedException("Werknemer niet gevonden"));
            if ("Manager".equals(doelWerknemer.getRol())
                    || "Admin".equals(doelWerknemer.getRol())) {
                throw new AccessDeniedException(
                        "Managers hebben geen toegang tot andere managers of admins");
            }
            return;
        }
    ...
    /**
     * Controleert of de ingelogde gebruiker toegang heeft tot een specifiek team. Admin/Manager:
     * altijd toegang Supervisor: alleen als ze lid zijn van dat team
     */
    ...
    /**
     * Controleert of de ingelogde gebruiker supervisor is van de opgegeven werknemer. D.w.z. de
     * ingelogde gebruiker is supervisor in een team waar de werknemer ook lid van is.
     */
    public boolean isSupervisorVanWerknemer(Integer werknemerId) {
        ...
        return teamwerknemerRepository.findByWerknemerId(ingelogdId).stream()
                ...
                .anyMatch(
                        tw ->
                                teamwerknemerRepository.existsByTeamIdAndWerknemerId(
                                        tw.getTeam().getId(), werknemerId));
    }

    /**
     * Controleert of de ingelogde gebruiker verlof/afwezigheid mag goedkeuren voor een werknemer.
     * Admin: altijd Manager: altijd behalve voor andere managers (hun verlof moet door een andere
     * manager goedgekeurd worden) Supervisor: alleen voor teamleden waar ze supervisor van zijn
     */
    public void assertMagGoedkeuren(Integer werknemerId) {
        ...
        throw new AccessDeniedException(
                "Geen rechten om verlof goed te keuren voor deze werknemer");
    }
