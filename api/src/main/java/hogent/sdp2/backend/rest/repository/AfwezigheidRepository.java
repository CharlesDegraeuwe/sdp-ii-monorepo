...
public interface AfwezigheidRepository extends JpaRepository<Afwezigheid, Integer> {
    ...
    @Query("SELECT COUNT(a) > 0 FROM Afwezigheid a WHERE a.werknemer.id = :werknemerId AND :vandaag BETWEEN a.startDatum AND a.eindDatum")
    boolean isWerknemerAfwezigOpDatum(@Param("werknemerId") Integer werknemerId, @Param("vandaag") LocalDate vandaag);
}