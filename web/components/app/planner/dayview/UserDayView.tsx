...
const UserDayView = (props: DayViewProps) => {
  const { huidigeDatum, afwezigheden, taken } = props;
  const opDag = afwezighedenOpDag(afwezigheden, huidigeDatum);
  const dagTaken = takenOpDag(taken, huidigeDatum).filter((t) => !t.afgewerkt);
...
UserDayView.displayName = 'UserDayView';
export default UserDayView;
