      ...
      <AppContainer>
        ...
        <div className="w-full h-full flex flex-col gap-6">
          <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4 w-full">
            ...
            <div className="w-full sm:w-fit flex flex-row gap-2 sm:gap-3 items-center justify-between sm:justify-start">
              <div className="flex gap-1 bg-gray-300/30 border border-gray-300/30 rounded-full p-1 shadow-sm">
                {(['maand', 'week', 'dag'] as View[]).map((v) => (
                  <button
                    key={v}
                    onClick={() => setView(v)}
                    className={`px-3 sm:px-5 py-2 rounded-full text-sm font-bold capitalize transition-all duration-300 ${
                      view === v
