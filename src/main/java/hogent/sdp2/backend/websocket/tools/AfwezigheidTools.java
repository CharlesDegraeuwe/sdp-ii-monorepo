...
public class AfwezigheidTools {
    ...
    @Tool(description = "Haal de afwezigheden op van de teamgenoten van de ingelogde gebruiker. Enkel beschikbaar voor supervisors en managers.")
    public Object getTeamAbsence(ToolContext ctx) {
        Integer userId = (Integer) ctx.getContext().get("userId");
        String role = (String) ctx.getContext().get("userRole");
        System.out.println("=== TOOL CALL === getTeamAbsence voor user " + userId + " met rol " + role);

        if (!SUPERVISOR_ROLES.contains(role)) {
            return "GEEN_TOEGANG: deze gebruiker heeft rol " + role +
    ...
    @Tool(description = "Haal de afwezigheden op van een specifieke werknemer op basis van zijn id. Enkel beschikbaar voor supervisors en managers.")
    public Object getSpecificUserAbsence(Integer werknemerId, ToolContext ctx) {
        String role = (String) ctx.getContext().get("userRole");
        System.out.println("=== TOOL CALL === getSpecificUserAbsence voor werknemer " + werknemerId + " met rol " + role);
