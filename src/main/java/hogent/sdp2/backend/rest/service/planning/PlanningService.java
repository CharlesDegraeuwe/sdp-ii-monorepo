...
public class PlanningService {
    ...
    public List<AfwezigheidsOverzichtDTO> geefPlanningVanWerknemer(Integer werknemerId, LocalDate van, LocalDate tot) {
        List<AfwezigheidsOverzichtDTO> resultaat = new ArrayList<>();

        var werknemer = afwezigheidRepository.findByWerknemerId(werknemerId).stream().findFirst()
        ...
        afwezigheidRepository.findByWerknemerId(werknemerId).stream()
                .filter(a -> !a.getEindDatum().isBefore(van) && !a.getStartDatum().isAfter(tot))
                .forEach(a -> resultaat.add(new AfwezigheidsOverzichtDTO(
                        werknemerId, finalVoornaam, finalNaam,
                        "Ziekte",
                        a.getStartDatum(), a.getEindDatum(),
                        null
                )));
