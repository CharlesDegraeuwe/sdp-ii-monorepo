...
export const TeamMemberCard = ({ lid, teamId }: Props) => {
  ...
  return (
    <div className="p-0.5">
      <div
        onClick={() => router.push(`/teams/${teamId}/leden/${lid.werknemerId}`)}
        className="group px-4 py-2 shadow-sm rounded-full text-sm flex flex-row gap-2 items-center justify-between min-h-13 transition bg-zinc-100 hover:bg-zinc-200 cursor-pointer"
      >
        ...
        <div className="flex flex-row items-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <button
            onClick={(e) => {
              e.stopPropagation();
              promoot(teamId, lid.werknemerId);
            }}
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              verwijder(teamId, lid.werknemerId);
            }}
